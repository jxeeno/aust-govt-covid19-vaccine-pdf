...
async function scrapeSAXI(data, aggLevel, url) {
            ...
            if(header[0] === 'State' && header[1] === headerCell){
                for(const r of table){
                    ...
                    const row = {
                        ...
                        AGE_LOWER: 15,
                        AGE_UPPER: 999,
                        AIR_FIRST_DOSE_PCT: Number(r[5].replace(/[^0-9\.]+/g, '')),
                        AIR_SECOND_DOSE_PCT: Number(r[6].replace(/[^0-9\.]+/g, '')),
                    ...
                    // row.AIR_INDIGENOUS_POPULATION = aggpop ? aggpop.population15plus : '';
