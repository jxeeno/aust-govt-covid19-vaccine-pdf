...
const PUBLICATION_JSON_DATA_PATH = 'docs/data/geo/';
...
const getPublications = async () => {
    ...
    for(const item of items){
        let {name, landingUrl, pdfUrl, filename} = item;
        ...
        let pdfData;
        if(item.type === 'lga'){
            pdfData = await scrapeLGA(pdfBuffer)
        }else if(item.type === 'sa3'){
        ...
        if(!pdfData.dataAsAt){
            ...
            vaccineDataPath = path.join(PUBLICATION_JSON_DATA_PATH, `${pdfData.dataAsAt}.${item.type}.json`);
